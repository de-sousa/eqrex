...
unionWordSearch :: (Eq sy, Eq st, Ord sy, Ord st) => Dfa st sy -> [LOL sy]
unionWordSearch fulldfa =
   [LOL str | (LOL str,bool) <- uWS dfa init (LOL []), bool]
      where
...
regExpFeedback :: Int -> RegExp Char -> RegExp Char -> MyResult (LOL Char)
...
applyRegExpFeedback :: String -> String -> Int -> MyResult (LOL Char)
